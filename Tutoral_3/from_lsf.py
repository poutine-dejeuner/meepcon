...
Si = mp.Medium(index=wg_index) # Effective permittivity for a Silicon waveguide with a thickness of 220nm
SiO2 = mp.Medium(index=bg_index)
delta = dx.to(ur.micrometers/ur.pixel) #(0.02)*ur.micrometers  size of a pixel (in μm) 20 nm in lumerical exp
# resolution = 20 # (pixels/μm)
resolution = 1/delta # pixels/μm
waveguide_width = (wg_width).to(ur.micrometer) #0.5 # (μm)

#The above settings are not compatible with the (101, 91) design shapes of the previous experiment. the values below replace the optimisation regions with compatible shapes
half_design_shape = (101,91)
design_sizex = 101*ur.pixel*dx
design_sizey = 91*ur.pixel*dx
...
design_region_height = 2*design_sizey.to(ur.micrometer) # (μm)

arm_separation = (out_wg_dist).to(ur.micrometer) #1.0  (μm) distance between arms center to center

ic(design_region_height*resolution)
waveguide_length = (source_wg_xmax - source_wg_xmin).to(ur.micrometer) #0.5  (μm)
pml_size = 1.0 # (μm)
